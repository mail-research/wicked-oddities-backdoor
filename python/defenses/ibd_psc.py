...
class IBD_PSC(Base):
    ...
    def _test(self, dataset):
        ...
        all_psc_score = all_psc_score[pred_correct_mask.cpu()]
...
def run_ibd_psc(args):
    ...
    valset = Subset(clean_dataset, range(2000))
    poisoned_dataset, _ = build_backdoor_dataset(1, args, is_train=False)
    ...
    detector = IBD_PSC(model, valset=valset)
    detector.test(clean_dataset, poisoned_dataset)